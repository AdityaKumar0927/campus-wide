...
| D-18 | **Design direction: "campus editorial".** Warm paper-toned OKLCH neutrals with one admin-configurable campus accent hue; a humanist sans for UI paired with a serif display face for headlines; hairline rules and generous whitespace instead of heavy shadows, gradients, or glassmorphism; content-first cards, no hero-with-purple-gradient. Phase 1 DoD includes an owner look review. | Brief §1.5 demands a fresh, non-template look; naming a direction makes it checkable. |
